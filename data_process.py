...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms):
        ...
        self.text_token_list = [text if len(text) < opt.word_length else text[0: opt.word_length] for text in self.text_token_list]
        self.text_to_id = [text_tokenizer.convert_tokens_to_ids(text_token) for text_token in tqdm(self.text_token_list, desc='convert text to id')]
    ...
    def __getitem__(self, index):
        ...
        text_token_ids = torch.tensor(self.text_to_id[index])
        return text_token_ids, image_origin, self.label_list[index]
...
class Collate():
    ...
    def _collate(self, batch_data):
        ...
        return text_to_id, torch.LongTensor(bert_attention_mask), image_origin, label
...
def data_process(opt, data_path, text_tokenizer, photo_path, data_type):
    ...
    dataset = SentenceDataset(opt, data_path, text_tokenizer, photo_path, transform_train if data_type == 1 else transform_test_dev)
